...
import LaunchFilters from "../../components/Filters/Filters";
...
export class LaunchProgram extends React.Component {
    ...
    filterData: [
      {
        ...
        data: new Array(new Date().getFullYear() - 2005)
          ...
          .map((a, i) => 2006 + i),
        activeItem:
          this.props.query["launch_year"] &&
          parseInt(this.props.query["launch_year"]),
      },
      {
        type: "launch_success",
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      prevState.dataLimit !== this.state.dataLimit
    ) {
      this.props.fetchLaunchData(
        `?limit=${this.state.dataLimit}${
          this.state.queryString && "&" + this.state.queryString
  ...
  onFilterApply = (category, value) => {
    ...
    const currentCategory = {
      ...this.state.filterData.find(
        (filterCategory) => filterCategory.type === category
      ),
    };
    ...
    this.setState((prevState) => {
      return {
        filterData: prevState.filterData.map((filterCategory) =>
          filterCategory.type === category ? currentCategory : filterCategory
        ),
